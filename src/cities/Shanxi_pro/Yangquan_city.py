...
class YangquanCrawler(PageBase):
    """
    xpath: get 获取目录页网页数据
    二级跳转：需要获取详情页的url
    xpath:目的数据在详情页中
    ...
    def __init__(self, is_headless=True):
        city_info = {'name': '阳泉市',
                     'province': 'Shanxi',
                     'total_items_num': 193 + 200,
                     'each_page_count': 10,
                     'base_url': 'http://data.yq.gov.cn/oportal/catalog/index?page={page_num}'
                     }
        ...
        self.headers = {
            ...
            "Cookie": "OPENSESSION=adc652d7-473e-4742-a28d-7ac4c35410ea", "Host": "data.yq.gov.cn",
            "Proxy-Connection": "keep-alive", "Referer": "http://data.yq.gov.cn/oportal/catalog/",
        ...
        for frame in frames_list:
            try:
                ...
                if not frame_url.startswith('http'):
                    frame_url = f'http://data.yq.gov.cn{frame_url}'
                else:
                    frame_url = frame_url
    ...
    def extract_page_data(self, session_page, files_type):
        ...
        update_time = session_page.ele('x://td[contains(text(),"最后更新时间")]/following-sibling::td').text

        open_conditions = session_page.ele('x://td[contains(text(),"开放状态")]/following-sibling::td').text
        data_volume = session_page.ele('x://td[contains(text(),"数据量")]/following-sibling::td').text if session_page.ele('x://td[contains(text(),"数据量")]/following-sibling::td') else 0
...
if __name__ == '__main__':
    page = YangquanCrawler(is_headless=True)
    page.run()
