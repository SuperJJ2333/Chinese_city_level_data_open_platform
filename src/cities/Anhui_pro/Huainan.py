...
class HuainanCrawler(PageBase):
    ...
    def __init__(self, is_headless=True):
        ...
        api_city_info = {'name': '淮南市_api',
                         'province': 'Anhui',
                         'total_items_num': 13 + 10,
                         'each_page_count': 10,
                         'base_url': 'https://sjzyj.huainan.gov.cn/odssite/govData/getGovDataList?dataType=2&fuzzy=&page={page_num}&size=10'
                         }
    ...
    def extract_page_data(self, json_data):
        ...
        for item in results:
            title = item.get('dataName', '')
            subject = item.get('dataDomainName', '')
            description = item.get('summary', '')
            source_department = item.get('pubDeptName', '')
...
if __name__ == '__main__':
    page = HuainanCrawler(is_headless=True)
    page.run()
