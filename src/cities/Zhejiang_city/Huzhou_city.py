...
class HuzhouCrawler(PageBase):
    ...
    def __init__(self, is_headless=True):
        city_info = {'name': '湖州市',
                     'province': 'Zhejiang',
                     'total_items_num': 2145 + 500,
                     'each_page_count': 10,
                     'base_url': 'http://data.huzhou.gov.cn/api/data/Item'
                     }

        super().__init__(city_info, is_headless)

        self.headers = {"Accept":"application/json, text/plain, */*","Accept-Encoding":"gzip, deflate","Accept-Language":"zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6","Content-Type":"application/json;charset=UTF-8","Cookie":"SERVERID=f00d01f2709b88b0e69ae58f2dc53f23|1719337533|1719337213","Host":"data.huzhou.gov.cn","Origin":"http://data.huzhou.gov.cn","Referer":"http://data.huzhou.gov.cn/open_data/dataset","User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 Edg/126.0.0.0"}
        self.params = '{"page":2,"pageSize":10,"centerCode":null,"areaName":[],"dataFormat":[],"updateCycle":[],"openLevel":[],"condition":"","deptCode":null,"deptName":"","sortCondition":null,"type":1,"tags":[]}'
    ...
    def extract_page_data(self, json_data):
        ...
        data = data['data']['list']  # Update the path according to the JSON structure
        models = []

        for item in data:
            title = item.get('catalogName', '')
            subject = item.get('keyAreasName', '')[0]  # Assuming trade name represents the subject
...
if __name__ == '__main__':
    page = HuzhouCrawler(is_headless=True)
    page.run()