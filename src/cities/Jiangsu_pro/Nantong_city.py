...
class NantongCrawler(PageBase):
    ...
    def extract_page_data(self, json_data, item_url):
        ...
        subject = item.get('resourceSubjectName', '')  # 使用新提供的字段名称
        ...
        access_count = int(item.get('scanNum', 0)) if item.get('scanNum') is not None else 0
        download_count = int(item.get('downNum', 0)) if item.get('downNum') is not None else 0
