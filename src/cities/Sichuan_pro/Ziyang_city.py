...
class ZiyangCrawler(PageBase):
    ...
    def __init__(self, is_headless=True):
        city_info = {'name': '资阳市',
                     'province': 'Sichuan',
                     'total_items_num': 3053 + 500,
                     'each_page_count': 10,
                     'base_url': 'http://data.ziyang.gov.cn/data/dataSet/findPage'}

        super().__init__(city_info, is_headless)

        self.headers = {"Accept":"application/json, text/javascript, */*; q=0.01","Accept-Encoding":"gzip, deflate","Accept-Language":"zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6","Connection":"keep-alive","Content-Length":"54","Content-Type":"application/x-www-form-urlencoded; charset=UTF-8","Cookie":"JSESSIONID=d05b5333-ff14-44ca-b33e-04c2ab674112","Host":"data.ziyang.gov.cn","Origin":"http://data.ziyang.gov.cn","Referer":"http://data.ziyang.gov.cn/data/catalog/toDataCatalog","User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 Edg/126.0.0.0","X-Requested-With":"XMLHttpRequest"}

        self.params = {'pageNo': '2', 'pageSize': '10', 'officeType': '0', 'cate': '2', 'sortType': 'DESC'}
    ...
    def process_views(self):
        ...
        for i in range(1, self.total_page_num):
            ...
            data['pageNo'] = str(i)
            session.post(url=self.base_url, proxies=self.proxies, headers=self.headers, data=data)
    ...
    def extract_page_data(self, json_data, item_url):
        ...
        results = data['list']  # 更新数据路径以适配新的JSON结构
        models = []

        for item in results:
            title = item.get('resTitle', '')
            subject = item.get('domainName', '')  # 未明确提供科目字段，假设使用domainName
...
if __name__ == '__main__':
    page = ZiyangCrawler(is_headless=True)
    page.run()