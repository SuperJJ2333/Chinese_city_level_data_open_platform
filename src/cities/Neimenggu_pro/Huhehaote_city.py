...
class HuhehaoteCrawler(PageBase):
    """
    一级跳转：get请求：获取json数据，在目录页获取目的数据
    """

    def __init__(self, is_headless=True):
        city_info = {'name': '呼和浩特市',
                     'province': 'NeiMenggu',
                     'total_items_num': 51 + 100,
                     'each_page_count': 10,
                     'base_url': 'https://data.huhhot.gov.cn/api/catalog?keyword=&resourceType=&categoryId=3&pageNum={page_num}&pageSize=10&orderBy=publishTime&dir=desc'}

        super().__init__(city_info, is_headless)

        self.headers = {"Accept": "application/json, text/plain, */*", "Accept-Encoding": "gzip, deflate, br, zstd",
                        "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6",
                        "Connection": "keep-alive", "Cookie": "isVisit=true", "Host": "data.huhhot.gov.cn",
                        "Referer": "https://data.huhhot.gov.cn/", "Sec-Fetch-Dest": "empty", "Sec-Fetch-Mode": "cors",
...
if __name__ == '__main__':
    page = HuhehaoteCrawler(is_headless=True)
    page.run()
