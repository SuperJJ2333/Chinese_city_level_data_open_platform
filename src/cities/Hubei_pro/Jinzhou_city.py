...
class JinzhouCrawler(PageBase):
    ...
    def __init__(self, is_headless=True):
        city_info = {'name': '荆州市',
                     'province': 'Hubei',
                     'total_items_num': 102 + 100,
                     'each_page_count': 10,
                     'base_url': 'http://data.jingzhou.gov.cn:10097/oportal/catalog/index?page={page_num}'
                     }
        ...
        self.headers = {
            ...
            "Cookie": "OPENSESSION=1ea8809f-d1b5-4743-9f73-05f25f404c16", "Host": "data.jingzhou.gov.cn:10097",
            "Referer": "http://data.jingzhou.gov.cn:10097/oportal/catalog/", "Upgrade-Insecure-Requests": "1",
        ...
        for frame in frames_list:
            try:
                ...
                if not frame_url.startswith('http'):
                    frame_url = f'http://data.jingzhou.gov.cn:10097{frame_url}'
                else:
                    frame_url = frame_url
    ...
    def extract_page_data(self, session_page, files_type):
        # Assuming session_page has loaded the HTML content
        frames = session_page.ele('x://*[@id="app"]/div[7]/div[2]/div/ul/li[1]/table')

        title = session_page.ele('x://div[@class="directory-body"]/div[@class="d-header"]/ul/li/h4').text
        subject = session_page.ele('x://div[@class="list-details"]/ul/li[2]/span[@class="text-primary"]').text
        description = frames.ele('x://tr[7]/td[2]').text
        source_department = session_page.ele('x://div[@class="list-details"]/ul/li[1]/span[@class="text-primary"]').text

        release_time = session_page.ele('x://div[@class="list-details"]/ul/li[3]/span[@class="text-primary"]').text
        update_time = session_page.ele('x://div[@class="list-details"]/ul/li[4]/span[@class="text-primary"]').text

        open_conditions = session_page.ele('x://*[@id="app"]/div[7]/div[1]/div/div/div/div[2]/ul[1]/li[5]/span').text
        data_volume = frames.ele('x://tr[1]/td[4]').text  # Assuming the data volume is in the first row, fourth column of a specific table
        file_type = [files_type]  # Assuming files_type is a parameter specifying the type of files
...
if __name__ == '__main__':
    page = JinzhouCrawler(is_headless=True)
    page.run()
