...
class HuanggangCrawler(PageBase):
    """
    xpath: get 获取目录页网页数据
    二级跳转：需要获取详情页的url
    xpath:目的数据在详情页中
    ...
    def __init__(self, is_headless=True):
        city_info = {'name': '黄冈市',
                     'province': 'Hubei',
                     'total_items_num': 174 + 200,
                     'each_page_count': 10,
                     'base_url': 'http://data.hg.gov.cn:3379/oportal/catalog/index?page={page_num}'
                     }
...
if __name__ == '__main__':
    page = HuanggangCrawler(is_headless=True)
    page.run()
