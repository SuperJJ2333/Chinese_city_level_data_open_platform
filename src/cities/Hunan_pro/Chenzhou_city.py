...
class ChenzhouCrawler(PageBase):
    """
    post：请求获取api数据
    直接在目录页获取目的数据
    """
    ...
    def __init__(self, is_headless=True):
        city_info = {'name': '郴州市',
                     'province': 'Hunan',
                     'total_items_num': 425 + 200,
                     'each_page_count': 8,
                     'base_url': 'http://113.219.136.14:36001/data/dataSet/findPage'
                     }

        super().__init__(city_info, is_headless)

        self.headers = {"Accept":"application/json, text/javascript, */*; q=0.01","Accept-Encoding":"gzip, deflate","Accept-Language":"zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6","Content-Type":"application/x-www-form-urlencoded; charset=UTF-8","Cookie":"JSESSIONID=498ac2a1-c14b-4297-a13f-bd5b37c2c863","Host":"113.219.136.14:36001","Origin":"http://113.219.136.14:36001","Proxy-Connection":"keep-alive","Referer":"http://113.219.136.14:36001/data/dataSet/toDataSet","User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36 Edg/125.0.0.0","X-Requested-With":"XMLHttpRequest"}

        self.params = {'pageNo': '1', 'pageSize': '8', 'sortType': 'DESC'}
    ...
    def extract_page_data(self, json_data):
        ...
        results = json.loads(data['dataList'])
        # 调整数据路径以匹配提供的JSON结构
        models = []

        for item in results['list']:
            title = item.get('resTitle', '')
            subject = item.get('subjectName', '其他')  # 提供默认值
...
if __name__ == '__main__':
    page = ChenzhouCrawler(is_headless=True)
    page.run()
